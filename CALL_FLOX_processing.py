...
#data_folder = "test_csv_no_unc" 
data_folder = "test_csv_unc"
#data_folder = "test_nc"
...
SIF_unc_MC = True
parallel = True
mask_time_doy = True #only for the nc_input=True case. If True, it applies a time and DOY mask to the data, to select only a subset of the data for processing. The mask is defined in src/FLOX_mask.py
...
if __name__ == "__main__":
    ...
    if nc_input:
        FLOX_processing_master_nc(data_path, data_nc, cov_path, uncertainty_as_input, SIF_unc_MC, parallel, short_case, mask_time_doy)
    else:
